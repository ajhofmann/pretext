...
import { getBuiltInBanks, loadCueFile } from './content.ts'
...
function readTextValue(value: string | undefined): string | undefined {
  if (value === undefined || value.trim() === '') return undefined
  return existsSync(value) ? readFileSync(value, 'utf-8') : value
}

function parseCuePaths(value: string | undefined): ContentCue[] {
  if (value === undefined || value.trim() === '') return []
  const parts = value.split(',').map(entry => entry.trim()).filter(Boolean)
  const cues: ContentCue[] = []
  for (let index = 0; index < parts.length; index++) {
    cues.push(...loadCueFile(parts[index]!))
  }
  return cues
}

function parseBanks(value: string | undefined): ContentBank[] {
  if (value === undefined || value.trim() === '') return []
  const builtIns = getBuiltInBanks()
  return value.split(',').map((entry, index) => {
    const trimmed = entry.trim()
    const builtIn = builtIns.find(bank => bank.id === trimmed)
    if (builtIn !== undefined) return builtIn
    const text = readTextValue(trimmed)
    return {
      id: `bank-${index}`,
      text: text ?? trimmed,
    }
...
export function buildConfigFromArgs(args: CliArgs, video: VideoShape = {}): Mp4ToAsciiConfig {
  ...
  return {
    ...
    content: {
      ...base.content,
      text: readTextValue(args['text-source'] ?? args['text']) ?? base.content.text,
      cues: parseCuePaths(args['subtitle-file'] ?? args['subtitles']),
      descriptions: parseCuePaths(args['description-file'] ?? args['descriptions']),
      banks: parseBanks(args['banks']),
