...
- **Commands**: See `DEVELOPMENT.md` for the full command list. The key daily commands are `bun test`, `bun run check` (typecheck + lint), `bun run build:package`, and `bun start` (dev server on port 3000).
